...
from fusionwam.shared.fs import ensure_dir
...
from fusionwam.shared.samplers import ResumableEpochSampler
from fusionwam.shared.video_io import save_mp4
from fusionwam.shared.video_metrics import pil_frames_to_video_tensor, video_psnr, video_ssim
...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        logger.info(
            ...
            (ds_plugin.deepspeed_config.get("zero_optimization", {}).get("stage", "unknown")
             if ds_plugin is not None else "none"),
    ...
    def save_checkpoint(self):
        ...
        # 2x that; on lean disks train with save_full_state=false (weights-only
        # checkpoints, crash restarts from step 0).
        if not bool(self.cfg.get("save_full_state", True)):
