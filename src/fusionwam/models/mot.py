...
from fusionwam.models.video_dit import flash_attention, modulate, rope_apply
...
class MoT(nn.Module):
    ...
    def prefill_video_cache(
        ...
        hidden_snapshots: dict = {}
        if hidden_layers is not None and 0 in hidden_layers:
            hidden_snapshots[0] = x.detach().clone()  # pre-block tokens (post pre_dit)
        for layer_idx in range(self.num_layers):
            ...
            if hidden_layers is not None and (layer_idx + 1) in hidden_layers:
                hidden_snapshots[layer_idx + 1] = x.detach().clone()
        if hidden_layers is not None:
            return kv_cache, hidden_snapshots
        if return_hidden:
            # Final-layer hidden states (post 30 blocks) — e.g. for anchor probing.
            return kv_cache, x
        return kv_cache
