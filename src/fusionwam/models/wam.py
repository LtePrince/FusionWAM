...
from fusionwam.shared.logging_config import get_logger

from fusionwam.models.action_dit import ActionDiT
from fusionwam.models.loading.loader import load_wan22_ti2v_5b_components
from fusionwam.models.mot import MoT
from fusionwam.models.scheduler import WanContinuousFlowMatchScheduler
...
class WAM(torch.nn.Module):
    ...
    def __init__(
        ...
        # Plan A: optionally keep the text encoder on a separate device (e.g. CPU)
        # so it does not occupy the main GPU. Must be set before `self.to(...)` below.
        self.text_encoder_device = (
            torch.device(text_encoder_device) if text_encoder_device is not None else None
        )
        # Cache of prompt -> (context, mask) on CPU; avoids recomputing the (slow when
        # offloaded) text-encoder forward for the constant instruction within an episode.
        self._prompt_emb_cache: dict[str, tuple[torch.Tensor, torch.Tensor]] = {}
    ...
    def from_wan22_pretrained(
        ...
        if video_dit_config is None:
            raise ValueError("`video_dit_config` is required for WAM.from_wan22_pretrained().")
    ...
    def to(self, *args, **kwargs):
        te_device = getattr(self, "text_encoder_device", None)
        if te_device is not None and self.text_encoder is not None:
            # Offload mode: temporarily detach the text encoder from the module
            # registry so the global `.to()` does not pull its ~11GB onto the main
            # (GPU) device, then keep it pinned to its own device.
            ...
            self.text_encoder.to(device=te_device)
    ...
    def encode_prompt(self, prompt: Union[str, Sequence[str]]):
        ...
        # When the text encoder is offloaded (e.g. to CPU), reuse a cached embedding for
        # an identical prompt instead of recomputing the (slow) forward every replan.
        cache_key = prompt if isinstance(prompt, str) else None
        if cache_key is not None and cache_key in self._prompt_emb_cache:
            emb, m = self._prompt_emb_cache[cache_key]
            return emb.to(device=self.device), m.to(device=self.device)
        ...
        if cache_key is not None:
            self._prompt_emb_cache[cache_key] = (prompt_emb.detach().cpu(), mask.detach().cpu())
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        if "context" not in sample or "context_mask" not in sample:
            raise ValueError(
                "WAM training requires `sample['context']` and `sample['context_mask']`."
            )
