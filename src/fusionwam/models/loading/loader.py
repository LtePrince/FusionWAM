...
from fusionwam.models.loading.io import ModelConfig, hash_model_file, load_state_dict
from fusionwam.models.loading.converters import (
    wan_video_vae_state_dict_converter,
)
from fusionwam.models.video_dit import WanVideoDiT
from fusionwam.models.text_encoder import HuggingfaceTokenizer, WanTextEncoder
from fusionwam.models.vae import WanVideoVAE38
...
def load_wan22_ti2v_5b_components(
    ...
    if load_text_encoder:
        te_device = text_encoder_device if text_encoder_device is not None else device
        if str(te_device) != str(device):
            logger.info("Offloading text encoder to %s (main device=%s).", te_device, device)
