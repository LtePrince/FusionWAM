...
class WAMJoint(WAM):
    """WAM variant where action attends to all video latent tokens."""
    ...
    def from_wan22_pretrained(cls, **kwargs):
        ...
        if not isinstance(video_dit_config, dict):
            raise ValueError(
                "`video_dit_config` must be provided as dict for WAMJoint."
            )
        if bool(video_dit_config.get("action_conditioned", False)):
            raise ValueError(
                "WAMJoint requires `video_dit_config['action_conditioned']=false`."
            )
